...
const PROFILE_DEMAND_THRESHOLD := 0.55
const ACCEPTABLE_PRICE_BASE := 0.45
const ACCEPTABLE_PRICE_PREFERENCE_SCALE := 0.70
...
static func can_sell_to(is_open: bool, stock: int, stall_slots: Array, current_item_id: String, current_price: int, customer_type: String, customer_profile: Dictionary = {}) -> Dictionary:
	if not is_open or stock <= 0:
		return {"bought": false, "reason": "没货了", "dialogue_event": "no_interest"}
	...
	if customer_type == PrototypeConstants.CUSTOMER_STUDENT:
		...
		if slot_price <= 3:
			return {"bought": false, "reason": "学生觉得有点贵", "dialogue_event": "price_reject", "item_id": str(cheapest_slot.get("item_id", current_item_id)), "price": slot_price}
		return {"bought": false, "reason": "学生买不起", "dialogue_event": "budget_reject", "item_id": str(cheapest_slot.get("item_id", current_item_id)), "price": slot_price}
	if customer_type == PrototypeConstants.CUSTOMER_WORKER:
		...
		return {"bought": false, "reason": "工人觉得贵", "dialogue_event": "price_reject", "item_id": str(cheapest_slot.get("item_id", current_item_id)), "price": slot_price}
	return {"bought": false, "reason": "顾客离开", "dialogue_event": "no_interest"}
...
static func _profile_decision(stall_slots: Array, current_item_id: String, customer_type: String, customer_profile: Dictionary) -> Dictionary:
	...
	for index in range(stall_slots.size()):
		...
		if preference < PROFILE_DEMAND_THRESHOLD:
			continue
		wanted_any = true
		var acceptable_price := clampi(int(floor(float(budget) * (ACCEPTABLE_PRICE_BASE + preference * ACCEPTABLE_PRICE_PREFERENCE_SCALE))), 1, budget)
	...
	if not wanted_any:
		return {"bought": false, "reason": "%s暂时不想买这些商品" % customer_label, "dialogue_event": "no_interest", "item_id": current_item_id, "price": ConfigLoader.get_base_sell_price(current_item_id)}
	if StallInventory.cheapest_price(stall_slots) > budget:
		var cheapest_slot := StallInventory.cheapest_stock_slot(stall_slots)
		return {"bought": false, "reason": "%s预算不够" % customer_label, "dialogue_event": "budget_reject", "item_id": str(cheapest_slot.get("item_id", current_item_id)), "price": int(cheapest_slot.get("price", ConfigLoader.get_base_sell_price(current_item_id)))}
	var cheapest_value_slot := StallInventory.cheapest_stock_slot(stall_slots)
	return {"bought": false, "reason": "%s觉得不划算" % customer_label, "dialogue_event": "price_reject", "item_id": str(cheapest_value_slot.get("item_id", current_item_id)), "price": int(cheapest_value_slot.get("price", ConfigLoader.get_base_sell_price(current_item_id)))}

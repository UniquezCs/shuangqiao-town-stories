...
func _route_for_mode(route_mode: String) -> Dictionary:
	for _attempt in range(12):
		...
		var route := _route_for_positions(residence, destination) if route_mode == CustomerSchedule.ROUTE_HOME_TO_DESTINATION else _route_for_positions(destination, residence)
		if not route.is_empty():
			return route
	if route_world != null:
		var destination := _destination_position()
		for residence_endpoint in _endpoints_by_id(route_world, home_endpoint_id):
			var residence := _endpoint_position(residence_endpoint)
			var route := _route_for_positions(residence, destination) if route_mode == CustomerSchedule.ROUTE_HOME_TO_DESTINATION else _route_for_positions(destination, residence)
			if not route.is_empty():
				return route
	return {}
...
func _endpoints_by_id(world: Node, endpoint_id: String) -> Array[Node2D]:
	...
	for node in get_tree().get_nodes_in_group("npc_endpoint"):
		if node is Node2D and node.is_inside_tree() and _node_belongs_to_world(node, world):
